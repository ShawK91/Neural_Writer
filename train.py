...
class Task_Book:
    def __init__(self, parameters):
        ...
        #model = Single_LSTM(parameters.num_input, parameters.num_hnodes, parameters.num_mem, parameters.num_output, self.n_vocab)
        #model = Single_MMU(parameters.num_input, parameters.num_hnodes, parameters.num_mem, parameters.num_output, self.n_vocab)
    ...
    def run_bprop(self, model):
        ...
        for epoch in range(1, self.params.num_epoch):
            ...
            if epoch % 5 == 0: torch.save(model, save_name)
    ...
    def get_data(self, seq_len):
        ...
        data_x = []; data_y = []
        iter_max = len(self.raw_text) - parameters.seq_len-1
        if iter_max > max_data: iter_max = max_data
